...
class AutomazioniFornitori:
    ...
    def trova_sezione(self, ws, titolo, max_search=500):
        """
        Cerca dinamicamente una sezione (es. 'FERRAMENTA', 'ILLUMINAZIONE') in colonna A.
        La struttura attesa è:
            riga N      -> titolo (es. 'FERRAMENTA')
            riga N+1    -> header (MOBILE | CODICE | DESCRIZIONE | ...)
            riga N+2... -> dati (anche con righe vuote in mezzo)
            riga finale -> 'TOTALE' in colonna A
        Restituisce (start_row, end_row): prima e ultima riga di dati (= TOTALE - 1).
        Restituisce (None, None) se la sezione non viene trovata.
        Se non trova TOTALE, usa l'inizio della sezione successiva o un fallback di 200 righe.
        """
        titolo_up = titolo.strip().upper()
        title_row = None
        
        # 1) Cerca il titolo in colonna A
        for r in range(1, max_search + 1):
            v = ws.range(f'A{r}').value
            if v and str(v).strip().upper() == titolo_up:
                title_row = r
                break
        ...
        # 2) Dati partono 2 righe sotto (salta il titolo + header)
        start_row = title_row + 2
        
        # 3) Cerca 'TOTALE' in colonna A → fine sezione
        end_row = None
        for r in range(start_row, max_search + 1):
            val_a = ws.range(f'A{r}').value
            if val_a and str(val_a).strip().upper() == "TOTALE":
                end_row = r - 1
                break
        
        # Fallback: se non trova TOTALE, usa max 200 righe
        if end_row is None:
            end_row = min(start_row + 200, max_search)
    ...
    def crea_fogli_fornitori(self):
        ...
        try:
            ...
            # Trova dinamicamente la sezione FERRAMENTA in colonna A
            start_row, end_row = self.trova_sezione(ws_main, "FERRAMENTA")
            
            if start_row is None:
                wb.close()
                os.remove(dest_path)
                messagebox.showwarning("Attenzione", "Sezione 'FERRAMENTA' non trovata in colonna A")
                return
            ...
            if not fornitori:
                wb.close()
                os.remove(dest_path)
                messagebox.showwarning("Attenzione", f"Nessun fornitore trovato nella sezione FERRAMENTA (righe {start_row}-{end_row})")
                return
    ...
    def crea_fogli_illuminazione(self):
        ...
        try:
            ...
            # Trova dinamicamente la sezione ILLUMINAZIONE in colonna A
            start_row, end_row = self.trova_sezione(ws_main, "ILLUMINAZIONE")
            
            if start_row is None:
                wb.close()
                os.remove(dest_path)
                messagebox.showwarning("Attenzione", "Sezione 'ILLUMINAZIONE' non trovata in colonna A")
                return
            ...
            if not fornitori:
                wb.close()
                os.remove(dest_path)
                messagebox.showwarning("Attenzione", f"Nessun fornitore trovato nella sezione ILLUMINAZIONE (righe {start_row}-{end_row})")
                return
    ...
    def raggruppa_sezione(self):
        ...
        try:
            ...
            for r in range(data_start, last_row + 1):
                ...
                # === MISURE MINIME DI TAGLIO ===
                # Se L < 350 -> L = 350 ; se H < 110 -> H = 110
                try:
                    L_num = float(L) if L not in (None, "") else None
                    if L_num is not None and L_num < 350:
                        L = 350
                except (ValueError, TypeError):
                    pass
                
                try:
                    H_num = float(H) if H not in (None, "") else None
                    if H_num is not None and H_num < 110:
                        H = 110
                except (ValueError, TypeError):
                    pass
    ...
    def elaborazione_completa(self):
        ...
        try:
            ...
            # Trova dinamicamente la sezione FERRAMENTA in colonna A
            ferr_start, ferr_end = self.trova_sezione(ws_main, "FERRAMENTA")
            
            if ferr_start is not None:
                for r in range(ferr_start, ferr_end + 1):
                    desc = str(ws_main.range(f'C{r}').value or "").strip()
                    forn = str(ws_main.range(f'E{r}').value or "").strip()
            ...
            # Trova dinamicamente la sezione ILLUMINAZIONE in colonna A
            ill_start, ill_end = self.trova_sezione(ws_ill, "ILLUMINAZIONE")
            
            if ill_start is not None:
                for r in range(ill_start, ill_end + 1):
